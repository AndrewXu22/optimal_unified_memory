...
for f1 in filenames:
    ...
    fp = open(f1)
    with open("cfd_time.csv", "a") as f:
        for i, line in enumerate(fp):
            if i == 4:
                kernel_time = line.split(" ")[7]
                line = ','.join([advice_name, benchmark_name, "cuda_compute_flux", kernel_time,  '\n'])
                #print line
                f.write(line)
            elif i == 5:
                kernel2_time = line.split(" ")[21]
                line = ','.join([advice_name, benchmark_name,  "cuda_time_step", kernel2_time, '\n'])
                #print line
                f.write(line)
        fp.close()