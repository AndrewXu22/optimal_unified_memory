#!/bin/bash -x
# binary 526 is corrupted. 
# this script will just restart from iterations after 526 (skip any iterations from 000 to 526)
...
if [ "$sum" -gt 526 ];then
  ...
  nvprof -u ms --log-file ./$RESULT_PATH/output_$i$j${k}_missile0.2M.log ./$BIN_PATH/cfd_$i$j$k ../../data/cfd/missile.domn.0.2M
  if [ $? -ne 0 ]; then
