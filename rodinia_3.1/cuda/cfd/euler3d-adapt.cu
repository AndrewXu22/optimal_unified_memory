 ...
 */
template <typename T>
T* alloc(int N, alloc_type memtype)
{
	T* t;
//	checkCudaErrors(cudaMalloc((void**)&t, sizeof(T)*N));
	t = (T*)xplacer_malloc( sizeof(T)*N,memtype);

	return t;
}

template <typename T>
void dealloc(T* array, alloc_type memtype)
{
//	checkCudaErrors(cudaFree((void*)array));
	xplacer_free((void*)array,memtype);
}

template <typename T>
void copy(T** dst, T* src, int N, bool isUnifiedMem)
{
//	checkCudaErrors(cudaMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToDevice));
	checkCudaErrors(xplacer_memcpy((void**)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToDevice, isUnifiedMem));
}

template <typename T>
void upload(T** dst, T* src, int N, bool isUnifiedMem)
{
//	checkCudaErrors(cudaMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyHostToDevice));
	checkCudaErrors(xplacer_memcpy((void**)dst, (void*)src, N*sizeof(T), cudaMemcpyHostToDevice, isUnifiedMem));
}

template <typename T>
void download(T** dst, T* src, int N, bool isUnifiedMem)
{
//	checkCudaErrors(cudaMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToHost));
	checkCudaErrors(xplacer_memcpy((void**)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToHost, isUnifiedMem));
}
...
void dump(float* variables, int nel, int nelr)
{
	...
	float* h_variables;
        int isManaged = isAllocated(variables);
        if(isManaged == 0)
          h_variables = new float[nelr*NVAR]; 
	download(&h_variables, variables, nelr*NVAR, isManaged);
  ...
  printf("WG size of kernel:initialize = %d, WG size of kernel:compute_step_factor = %d, WG size of kernel:compute_flux = %d, WG size of kernel:time_step = %d\n", BLOCK_SIZE_1, BLOCK_SIZE_2, BLOCK_SIZE_3, BLOCK_SIZE_4);
        ...
        float* h_normals;
	{
		...
		nelr = BLOCK_SIZE_0*((nel / BLOCK_SIZE_0 )+ std::min(1, nel % BLOCK_SIZE_0));
                ...
                alloc_type memtype = Managed;
                printf("alloc h_areas\n");
#if adv1==0
		h_areas = new float[nelr];
#else
                h_areas = (float*) xplacer_malloc(sizeof(float)*nelr,memtype);
#endif
#if adv1==2
      cudaMemAdvise(h_areas,sizeof(float)*nelr,cudaMemAdviseSetReadMostly, 0);
#elif adv1==3
      cudaMemAdvise(h_areas,sizeof(float)*nelr, cudaMemAdviseSetPreferredLocation, 0);
#elif adv1==4
      cudaMemAdvise(h_areas,sizeof(float)*nelr, cudaMemAdviseSetAccessedBy, 0);
#elif adv1==5
      cudaMemAdvise(h_areas,sizeof(float)*nelr, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
#elif adv1==6
      cudaMemAdvise(h_areas,sizeof(float)*nelr, cudaMemAdviseSetAccessedBy, cudaCpuDeviceId);
#endif
		
                printf("alloc h_elements_surrounding_elements\n");
#if adv2==0
		h_elements_surrounding_elements = new int[nelr*NNB];
#else
		h_elements_surrounding_elements = (int*) xplacer_malloc(sizeof(int)*nelr*NNB,memtype);
#endif
#if adv2==2
        cudaMemAdvise(h_elements_surrounding_elements,sizeof(float)*nelr*NNB,cudaMemAdviseSetReadMostly, 0);
#elif adv2==3
 	 cudaMemAdvise(h_elements_surrounding_elements,sizeof(float)*nelr*NNB, cudaMemAdviseSetPreferredLocation, 0);
#elif adv2==4
     cudaMemAdvise(h_elements_surrounding_elements,sizeof(float)*nelr*NNB, cudaMemAdviseSetAccessedBy, 0);
#elif adv2==5
 	 cudaMemAdvise(h_elements_surrounding_elements,sizeof(float)*nelr*NNB, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
#elif adv2==6
     cudaMemAdvise(h_elements_surrounding_elements,sizeof(float)*nelr*NNB, cudaMemAdviseSetAccessedBy, cudaCpuDeviceId);
#endif
		
                printf("alloc h_normals\n");
#if adv3==0
		h_normals = new float[nelr*NDIM*NNB];
#else
		h_normals = (float*) xplacer_malloc(sizeof(float)*nelr*NDIM*NNB,memtype);
#endif
#if adv3==2
      cudaMemAdvise(h_normals,sizeof(float)*nelr*NDIM*NNB,cudaMemAdviseSetReadMostly, 0);
#elif adv3==3
 	cudaMemAdvise(h_normals,sizeof(float)*nelr*NDIM*NNB, cudaMemAdviseSetPreferredLocation, 0);
#elif adv3==4
      cudaMemAdvise(h_normals,sizeof(float)*nelr*NDIM*NNB, cudaMemAdviseSetAccessedBy, 0);
#elif adv3==5
 	cudaMemAdvise(h_normals,sizeof(float)*nelr*NDIM*NNB, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
#elif adv3==6
      cudaMemAdvise(h_normals,sizeof(float)*nelr*NDIM*NNB, cudaMemAdviseSetAccessedBy, cudaCpuDeviceId);
#endif
		...
		if(!isAllocated(h_areas))
		  areas = alloc<float>(nelr, GPU);
		upload<float>(&areas, h_areas, nelr, isAllocated(h_areas));

		if(!isAllocated(h_elements_surrounding_elements))
		  elements_surrounding_elements = alloc<int>(nelr*NNB, GPU);
		upload<int>(&elements_surrounding_elements, h_elements_surrounding_elements, nelr*NNB, isAllocated(h_elements_surrounding_elements));

		if(!isAllocated(h_normals))
		  normals = alloc<float>(nelr*NDIM*NNB, GPU);
		upload<float>(&normals, h_normals, nelr*NDIM*NNB, isAllocated(h_normals));
	...
	float* variables = alloc<float>(nelr*NVAR, GPU);
	initialize_variables(nelr, variables);

	float* old_variables = alloc<float>(nelr*NVAR, GPU);   	
	float* fluxes = alloc<float>(nelr*NVAR, GPU);
	float* step_factors = alloc<float>(nelr, GPU); 
	...
	cudaMemset( (void*) step_factors, 0, sizeof(float)*nelr );
        //memset(step_factors,0, sizeof(float)*nelr);
	...
	std::cout << "Starting..." << std::endl;
        ...
        //cudaMemPrefetchAsync(areas,nelr*sizeof(float),0,0);
        //cudaMemPrefetchAsync(normals,nelr*NDIM*NNB*sizeof(float),0,0);
        //cudaMemPrefetchAsync(elements_surrounding_elements,nelr*NNB*sizeof(int),0,0);
	...
	{
		copy<float>(&old_variables, variables, nelr*NVAR, false);
...
//	std::cout << "Saved solution..." << std::endl;
	...
	if(isAllocated(areas))
	  dealloc<float>(areas, Managed);
        else
	  dealloc<float>(areas, GPU);
	if(isAllocated(elements_surrounding_elements))
	  dealloc<int>(elements_surrounding_elements, Managed);
        else
	  dealloc<int>(elements_surrounding_elements, GPU);
	if(isAllocated(normals))
	  dealloc<float>(normals, Managed);
        else
	  dealloc<float>(normals, GPU);
	

	dealloc<float>(variables, GPU);
	dealloc<float>(old_variables, GPU);
	dealloc<float>(fluxes, GPU);
	dealloc<float>(step_factors, GPU);
