...
{
// Using UM, just do a pointer copy
...
// Using UM
                float* h_areas;
                checkCudaErrors(cudaMallocManaged((void**)&h_areas, sizeof(float)*nelr));          

//		int* h_elements_surrounding_elements = new int[nelr*NNB];
// Using UM
                int* h_elements_surrounding_elements;
                checkCudaErrors(cudaMallocManaged((void**)&h_elements_surrounding_elements, sizeof(int)*nelr*NNB));         
 
//		float* h_normals = new float[nelr*NDIM*NNB];
// using UM
                float* h_normals;
                checkCudaErrors(cudaMallocManaged((void**)&h_normals, sizeof(float)*nelr*NDIM*NNB));         
...
// Using UM, replace pointer to UM memory
...
//			compute_flux(nelr, elements_surrounding_elements, normals, variables, fluxes);
			compute_flux(nelr, h_elements_surrounding_elements, h_normals, variables, fluxes);
			getLastCudaError("compute_flux failed");			
...
// Dealloc the UM
	dealloc<float>(h_areas);
	dealloc<int>(h_elements_surrounding_elements);
	dealloc<float>(h_normals);
