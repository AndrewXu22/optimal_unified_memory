...
//	host_IMGVF_array = (float **) malloc(sizeof(float *) * num_cells);
	host_I_array = (float **) xplacer_malloc(sizeof(float *) * num_cells, Managed);
	host_IMGVF_array = (float **) xplacer_malloc(sizeof(float *) * num_cells, Managed);
...
//	host_n_array = (int *) malloc(sizeof(int) * num_cells);
	host_m_array = (int *) xplacer_malloc(sizeof(int) * num_cells,Managed);
	host_n_array = (int *) xplacer_malloc(sizeof(int) * num_cells,Managed);
	...
	host_I_all = (float *) xplacer_malloc(total_mem_size,Managed);
...
//	cudaMemcpy(device_IMGVF_all, host_I_all, total_mem_size, cudaMemcpyHostToDevice);
	xplacer_memcpy((void**)&device_I_all, host_I_all, total_mem_size, cudaMemcpyHostToDevice,true);
	xplacer_memcpy((void**)&device_IMGVF_all, host_I_all, total_mem_size, cudaMemcpyHostToDevice,true);
...
//	cudaMemcpy(device_IMGVF_array, host_IMGVF_array, num_cells * sizeof(float *), cudaMemcpyHostToDevice);
	xplacer_memcpy((void**)&device_I_array, host_I_array, num_cells * sizeof(float *), cudaMemcpyHostToDevice,true);
	xplacer_memcpy((void**)&device_IMGVF_array, host_IMGVF_array, num_cells * sizeof(float *), cudaMemcpyHostToDevice,true);
...
//	cudaMemcpy(device_n_array, host_n_array, num_cells * sizeof(int), cudaMemcpyHostToDevice);
	xplacer_memcpy((void**)&device_m_array, host_m_array, num_cells * sizeof(int), cudaMemcpyHostToDevice,true);
	xplacer_memcpy((void**)&device_n_array, host_n_array, num_cells * sizeof(int), cudaMemcpyHostToDevice,true);
}
...
//	free(host_I_array);
//	free(host_I_all);
	xplacer_free(host_m_array,Managed);
	xplacer_free(host_n_array,Managed);
	xplacer_free(host_IMGVF_array,Managed);
	xplacer_free(host_I_array,Managed);
	xplacer_free(host_I_all,Managed);
}
